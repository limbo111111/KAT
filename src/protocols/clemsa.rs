...
const TE_SHORT: u32 = 385;
const TE_LONG: u32 = 2695;
...
impl ProtocolDecoder for ClemsaDecoder {
    ...
    fn feed(&mut self, level: bool, duration_us: u32) -> Option<DecodedSignal> {
        match self.step {
            DecoderStep::Reset => {
                if !level && duration_diff!(duration_us, TE_SHORT * 51) < TE_DELTA * 25 {
                ...
                if !level {
                    if duration_diff!(self.te_last, TE_SHORT) < TE_DELTA &&
                       duration_diff!(duration_us, TE_LONG) < TE_DELTA * 3 {
                        ...
                        self.add_bit(1);
                        self.step = DecoderStep::SaveDuration;
                    } else if duration_diff!(duration_us, TE_SHORT * 51) < TE_DELTA * 25 {
                        if duration_diff!(self.te_last, TE_SHORT) < TE_DELTA {
                            self.add_bit(0);
                        } else if duration_diff!(self.te_last, TE_LONG) < TE_DELTA * 3 {
                            self.add_bit(1);
                        ...
                        if self.decode_count_bit == MIN_COUNT_BIT_FOR_FOUND {
                            ...
                            let serial = (data >> 2) & 0xFFFF;
                            let btn = (data & 0x03) as u8;
...
impl Default for ClemsaDecoder {
    fn default() -> Self {
        Self::new()
    }
}
