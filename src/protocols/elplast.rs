...
const TE_SHORT: u32 = 230;
const TE_LONG: u32 = 1550;
const TE_DELTA: u32 = 160;
const MIN_COUNT_BIT_FOR_FOUND: usize = 18;
...
impl ProtocolDecoder for ElplastDecoder {
    ...
    fn feed(&mut self, level: bool, duration_us: u32) -> Option<DecodedSignal> {
        match self.step {
            DecoderStep::Reset => {
                if !level && duration_diff!(duration_us, TE_LONG * 8) < TE_DELTA * 13 {
                    self.decode_data = 0;
                    self.decode_count_bit = 0;
                ...
                if !level {
                    if duration_diff!(self.te_last, TE_LONG) < TE_DELTA &&
                       duration_diff!(duration_us, TE_SHORT) < TE_DELTA {
                        ...
                        self.add_bit(0);
                        self.step = DecoderStep::SaveDuration;
                    } else if duration_diff!(duration_us, TE_LONG * 8) < TE_DELTA * 13 {
                        if duration_diff!(self.te_last, TE_LONG) < TE_DELTA {
                            self.add_bit(1);
                        }
    ...
    fn encode(&self, decoded: &DecodedSignal, _button: u8) -> Option<Vec<LevelDuration>> {
        let mut upload = Vec::with_capacity(decoded.data_count_bit * 2);

        for i in (0..decoded.data_count_bit).rev() {
            if (decoded.data >> i) & 1 == 1 {
                upload.push(LevelDuration::new(true, TE_LONG));
                if i == 0 {
                    upload.push(LevelDuration::new(false, TE_LONG * 8));
                } else {
...
impl Default for ElplastDecoder {
    fn default() -> Self {
        Self::new()
    }
}
